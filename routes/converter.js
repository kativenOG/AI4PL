...
converterRoutes.post('/convert', (req, res) => {
  exec("echo \"" + req.body.data+ "\" > test.smv",
    function(error, stdout, stderr) {
	 ...
	 exec('wine wsynth.exe -model_type dn -out_type dot -algo agaf_then_acyclic_preferences -agaf states -mono -dynamic -reachability_analysis ./test.smv &> output.dot && cat output.dot',
          function(error, stdout, stderr) {
            if (error != null) {
              console.log("ERRORE: " + error);
              return res.status(500).json({ Error: error });
